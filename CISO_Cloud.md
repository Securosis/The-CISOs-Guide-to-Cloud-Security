...
* With SAML, users won't have a username and password with the cloud provider. The only way to log in is to first authenticate to your directory server, which then provides (invisible to the user) a token to allow access to the cloud provider. Users will need to be in the office or on a VPN since they don't even have a username or password for the service.
